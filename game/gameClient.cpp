...
{
    //ClientSideShape* clientSideShape = new ClientSideShape(mSceneMgr,"jay" + index,0,0,0,"sinbad.mesh");
	//clientSideShape->getSceneNode()->scale(30,30,30);
	//client->mPlayer = clientSideShape;
    Sinbad* sinbad = new Sinbad(client,mSceneMgr,"jay" + index,0,0,0,"sinbad.mesh");
	sinbad->getSceneNode()->scale(30,30,30);
	client->mPlayer = sinbad;
}
...
{
    Sinbad* sinbad = new Sinbad(client,mSceneMgr,"silentBob" + index,0,0,0,"sinbad.mesh");
	sinbad->getSceneNode()->scale(30,30,30);
	client->mServerPlayer = sinbad;
}
...
void GameClient::ReadDeltaMoveCommand(dreamMessage *mes, ClientSideClient *client)
{
	...
	if(flags & CMD_KEY)
	{
		...
		LogString("Client %d: Read key %d", client->index, client->command.mKey);
	}
	...
	client->serverFrame.mVelocityOld.x = client->serverFrame.mVelocity.x;
	client->serverFrame.mVelocityOld.z = client->serverFrame.mVelocity.z;
	...
	//LogString("mVelocity.y %f", client->serverFrame.mVelocity.y);
